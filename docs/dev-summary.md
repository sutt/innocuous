...
Update centered around testing infrastructure and minor updates to docs.

Developing Mock LLM instances that output synthetic tokens (like *Aa, Ba, Ca, ...* or *a1, b2, c3, ...*) or replay tokens recorded from a previous inference iterations. This allows us to run quick/light tests end-to-end tests without loading weights/actually doing inference (again).

**Interesting design pattern:** the tests pattern built up on synthetic tokens in *mock-llm-test* get's re-implemented with the more complex replay tests in *load-log-test*. And the *mock-llama-backtrack* spec/solution demonstrates pretty interesting testing logic.

| Task File | Contents (truncated) | Accepted SHA | Diffs | Test Diffs | Notes |
|------|-------------|---------|-------------|------------|-----------|
| [mock-llama-backtrack.md](../.public-agdocs/specs/mock-llama-backtrack.md) | Add extra patching logic/methods in tests/test_replay_v1:test_gen_1 to account for potential backtracking in decode for resetting iteration number in MockLlama to align with recorded-logits logs key. | [62ae7bb](https://github.com/sutt/innocuous/commit/62ae7bb) |  |  | Tricky logic but succeeds. Uses fix the xfail pattern. |
| [load-log-test.md](../.public-agdocs/specs/load-log-test.md) | Implement a .load(log_file=) method in stego_llm.log to load JSON log file data, and proc_gen_tokens_v3 in stego_llm.llm.mock for replaying logits. Add test_encode_decode_simulation_v3 in tests/test_simulate_llm. | [85e4859](https://github.com/sutt/innocuous/commit/85e4859) |  |  | Very useful, part 2 of 2 continued from previous. |
| [logging-toks.md](../.public-agdocs/specs/logging-toks.md) | Store dictionary of tokens for each step and dump full log at end. Add CLI flag --log-file with optional LOGFILE argument. Datastructure: iteration number -> top_logits dictionary. Only implement for encode initially. | [67b22ed](https://github.com/sutt/innocuous/commit/67b22ed) |  |  | Straight forward feat, decent starter class |
| [mock-llm-test.md](../.public-agdocs/specs/mock-llm-test.md) | Apply the logic of test_integration:test_encode_decode_integration but use mock llama format from test_simulate_llm instead of actual LLM inference. Include debugging/logging outputs. | [b5fbfea](https://github.com/sutt/innocuous/commit/b5fbfea) |  |  | Needed a vital correction of patch logic in [7501c9d](https://github.com/sutt/innocuous/commit/7501c9d). |
| [mock-llama-a.md](../.public-agdocs/specs/mock-llama-a.md) | Build sophisticated mock system for Llama object from llama-cpp-python package. Mock create_llm_client and get_token_probabilities methods. Create tests/test_simulate_llm.py with test proving mocking works for ten tokens. | [8d6ed51](https://github.com/sutt/innocuous/commit/8d6ed51) |  |  | Generates useful bolierplate / exercise type solution |
| [fill-md-tbl.md](../.public-agdocs/specs/fill-md-tbl.md) | Edit docs/tables/how-it-works-v1.md to add more information to "Table v1" and "Table v2" based on supplied data. Use patterns from "Already Filled Data (examples)" and truncate logprobs to 4 significant digits. | [2cdc0de](https://github.com/sutt/innocuous/commit/2cdc0de) |  |  | Useful for complex editing within markdown tables.  |
